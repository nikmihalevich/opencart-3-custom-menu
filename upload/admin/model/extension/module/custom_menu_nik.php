...
class ModelExtensionModuleCustomMenuNik extends Model
{
    ...
    public function addMenuItem($data) {
        foreach ($data['menu_item'] as $language_id => $value) {
            if(!empty($value['name'])) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "custom_menu_items SET module_id = '" . (int)$data['module_id'] . "', `language_id` = '" . (int)$language_id . "', `parent_id` = '" . (int)$value['parent_id'] . "', `name` = '" . $this->db->escape($value['name']) . "', `link` = '" . $this->db->escape($value['link']) . "'");
            }
            $menu_item_id = $this->db->getLastId();
            if (isset($value['image'])) {
                $this->db->query("UPDATE " . DB_PREFIX . "product SET image = '" . $this->db->escape($value['image']) . "' WHERE `id` = '" . (int)$menu_item_id . "'");
            }
        }
    ...
    public function editMenuItem($product_id, $data) {
